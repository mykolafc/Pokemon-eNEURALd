...
function readMonData(address)
    ...
    movebytes = {ss1[0] & 0xFFFF, ss1[0] >> 16, ss1[1] & 0xFFFF, ss1[1] >> 16}
    mon.pp = {ss1[2] & 0xFF, (ss1[2] >> 8) & 0xFF, (ss1[2] >> 16) & 0xFF, ss1[2] >> 24}
    mon.moves = {}

    for i, movebyte in ipairs(movebytes) do
        mon.moves[i] = movebyte + 1
    end
...
function getMonType(mon)
    local types = {}
    local name = PokemonNames[mon.species]

    table.insert(types, TypeByMon[name][1])
    if TypeByMon[name][2] ~= nil then
        table.insert(types, TypeByMon[name][2])
    else
        table.insert(types, Types.NONE)
    end
...
function getInputs()
    ...
    for i = 1, #party do
        table.insert(inputs, party[i].species)
        table.insert(inputs, party[i].hp)
        table.insert(inputs, party[i].maxHP)
        table.insert(inputs, party[i].attack)
        table.insert(inputs, party[i].defense)
        table.insert(inputs, party[i].speed)
        table.insert(inputs, party[i].spAttack)
        table.insert(inputs, party[i].spDefense)
        -- table.insert(inputs, party[i].heldItem)
        table.insert(inputs, party[i].types[1])
        table.insert(inputs, party[i].types[2])

        table.insert(inputs, party[i].moves[1])
        table.insert(inputs, party[i].moves[2])
        table.insert(inputs, party[i].moves[3])
        table.insert(inputs, party[i].moves[4])
    end
    for i = 1, empty do
        table.insert(inputs, -1) -- absolute spaghetti code here... I'm sorry
    ...
    table.insert(inputs, opponent.hp / opponent.maxHP)
    table.insert(inputs, opponent.types[1])
    table.insert(inputs, opponent.types[2])
    table.insert(inputs, opponent.species)
    -- table.insert(inputs, getBag())
...
function evaluateNetwork(network, inputs)
    ...
    local highestOutput = 1
    for o = 1, Outputs do
        if network.neurons[MaxNodes + o].value >= network.neurons[MaxNodes + highestOutput].value then
            highestOutput = o
        end
    end
...
function initializeRun()
    console.writeline("Initializing a run")
...
function evaluateCurrent()
    ...
    if selection < 5 then
        useMove(lead, lead.moves[selection])
    elseif selection < 9 then
        console.writeline("Wanted to switch to party member number " .. selection - 3)
        timeout = 0
        console.writeline("Timeout set to " .. timeout)
        -- switchMon(selection - 3)
    else
        console.writeline("Nothing being called")
        timeout = 0
        console.writeline("Timeout set to " .. timeout)
    end
...
while true do
    ...
    if PrevHealth == opponent.hp then
        timeout = timeout - 1
    else
        timeout = TimeoutConstant
        PrevHealth = opponent.hp
    end

    evaluateCurrent() -- Compute the inputs and output the corresponding commands
    ...
    for i = 1, #party do
        ...
        if party[i].status ~= 0 then
            statusPenalty = statusPenalty + 50
        end
    end
    ...
    fitness = fitness - statusPenalty
    genome.fitness = fitness

    if opponent.hp == 0 or partyHP == 0 or timeout < 1 then
        ...
        console.writeline("Going to initialize run for Species: " .. pool.currentSpecies .. " Genome: " ..
                              pool.currentGenome)
        initializeRun()
    end
